...
object VoiceLog {
    ...
    private fun describeServerEvent(type: String, json: JSONObject): String = when (type) {
        ...
        "response.output_audio.delta" -> "bytes=${json.optString("delta").length}"
        "response.output_audio_transcript.delta" -> "delta=${truncate(json.optString("delta"))}"
        "response.output_audio_transcript.done" -> "transcript=${truncate(json.optString("transcript"))}"
